...
class DiagnosticsTab(QWidget):
    ...
    def _make_stat_block(self, label: str, value: str = "0", color: str = Theme.ACCENT_PRIMARY):
        ...
        layout.setSpacing(2)
        lbl = QLabel(label)
        lbl.setStyleSheet(f"color: {Theme.TEXT_MUTED}; font-size: 9px; font-weight: 500; background: transparent;")
        layout.addWidget(lbl)
        val = QLabel(value)
        val.setObjectName("diag_value")
        val.setStyleSheet(f"color: {color}; font-size: 22px; font-weight: 600; background: transparent;")
